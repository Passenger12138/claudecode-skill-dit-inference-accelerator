...
> **⚠️ 重要指令：Claude 必须严格按照以下 8 个步骤依次执行，不得跳过任何步骤！**
>
> 每一步都必须：
> 1. 向用户展示选项
> 2. 使用 AskUserQuestion 工具询问用户选择
> 3. 等待用户回答后再进入下一步
> 4. 只有完成所有 8 步后才能生成最终代码

```
步骤 1：分析模型组件 → 【必须】展示组件分析结果
    ↓
步骤 2：询问是否 FP8 量化 → 【必须】使用 AskUserQuestion 询问
    ↓
步骤 3：询问流式轮转策略 → 【必须】使用 AskUserQuestion 询问
    ↓
步骤 4：询问是否有蒸馏 LoRA → 【必须】使用 AskUserQuestion 询问
    ↓
步骤 5：询问 Attention 加速方式 → 【必须】使用 AskUserQuestion 询问
    ↓
步骤 6：询问是否启用 Torch 计算加速 → 【必须】使用 AskUserQuestion 询问
    ↓
步骤 7：确认是否有其他需求 → 【必须】使用 AskUserQuestion 询问
    ↓
步骤 8：生成完整配置代码 → 只有完成前 7 步后才能执行
```

---

## 步骤 1：分析 Pipeline 组件【必须执行】
...
## 步骤 2：询问是否需要 FP8 量化【必须执行】

**Claude 必须使用 AskUserQuestion 工具询问用户：**
>
...
## 步骤 8：生成完整配置代码【只有完成步骤 1-7 后才能执行】

**⚠️ Claude 必须确认已完成前 7 步并获得用户所有选择后，才能生成代码。**
